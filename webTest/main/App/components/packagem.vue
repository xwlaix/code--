<template>
  ...
  <div class="weui-tab body-margin">
    <div class="weui-navbar">
      <div class="weui-navbar__item " v-bind:href="'#'+ item.url" v-for="(item,key,index) in title" @click="activeTitle(item,key,index)" v-bind:class="[index==0 ? 'weui-bar__item--on' : '']">
        {{ item.title }}
    ...
    <div class="weui-tab__bd " id="template">
      ...
      <div id="tab1" class="weui-tab__bd-item  add weui-tab__bd-item--active" v-if="libArr.tab1.length>0">
        <div class="weui-panel__hd package_title">
          <span v-bind:class="[libArr.tab3.length>0 ? '' : 'hidden']">普通套餐：</span>
        </div>
        <div class="rechargeSingleMonthPage" v-for="(tab1, index) in libArr.tab1" @click="active('tab1', index)">
          <div class="content-renzheng content-renzheng_left " v-bind:class="[tab1.active==true ? 'content_active' : '']">
      ...
      <div class="weui-tab__bd-item " id='tab2' v-bind:class="[libArr.tab1.length>0 ? '' : 'weui-tab__bd-item--active']">
        <div class="weui-panel weui-panel_access" v-if="libArr.tab3.length>0">
          <div class="weui-panel__hd package_title title_one" v-bind:class="[libArr.tab3.length>0 ? '' : 'hidden']">推荐套餐：
          </div>
          <div class="weui-panel__bd " v-for="(tab1,key,index) in libArr.tab3">
            <div @click="active('tab3', index)" class="weui-media-box weui-media-box_appmsg package_content" style="height: auto;">
        ...
        <div class="weui-panel weui-panel_access margin">
          ...
          <div class="weui-panel__bd " v-for="(tab1, index) in libArr.tab2">
            <div href="" class="weui-media-box weui-media-box_appmsg package_content" style="height: auto;" @click="active('tab2', index)" v-bind:class="[tab1.active==true ? 'active' : '']">
  ...
  beforeCreate: function() {
    if (sessionStorage.getItem("needFresh") != "true") {
      if (sessionStorage.getItem("MicroMessenger") == "true") {
        c.$get("?r=wms/personal/get-wx-info", function(res) {
          if (res.code == 0) {
            $.toptip("获取用户信息失败,请重新查询", 2000, "error");
            return;
          }
            ...
            c.$post(
              ...
              function(res) {
                if (res.code == 0) {
                  ...
                  localStorage.setItem(cardNo, "true");
    ...
    function loadCard(res, lib) {
      $.each(res.data, function(i, data) {
        ...
        data.active = false;
        if (data.is_dis == 1) {
          if (res.data[i].id != _this.pid && data.type == 1) {
