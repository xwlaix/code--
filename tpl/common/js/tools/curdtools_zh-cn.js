 ...
 * @return
 */
function deleteALLSelect(title,url,gname,alertMsg) {
    ...
    if (rows.length > 0) {
    	...
    	$.dialog.confirm(alertMsg, function(r) {
		   if (r) {
...
function openuploadwinBywh(title, url,name,width, height) {
	width = width?width:700;
	height = height?height:400;
	gridname=name;
	$.dialog({
 ...
 * 格式化批量卡号操作
 */
function formatBatchCardNo(cardNo){
	// 格式化卡号
 ...
 * 格式化批量ICCID操作(数字带字母)
 */
function formatBatchCardICCID(cardNo){
  	cardNo = cardNo.replace(/\s+/g, ",");
  	cardNo = cardNo.replace(/[\r\n]/g, ",");
  	cardNo = cardNo.replace(/[^A-Za-z_0-9,]*/g, "");
  	cardNo = cardNo.replace(/[,]+/g, ",");
  	cardNo = cardNo.replace(/^[^A-Za-z_0-9]+/g, "");
  	cardNo = cardNo.replace(/[^A-Za-z_0-9]+$/g, "");
  	return cardNo;
