      ...
      <el-collapse-item id="packageInfo" title="套餐信息" name="2" >
        ...
        <el-table
          ...
          <el-table-column align="center" label="开始时间" min-width="120">
            <template slot-scope="scope">
              <span>{{ scope.row.starttime ?scope.row.starttime:''| scope.row.starttime }}</span>
            </template>
          </el-table-column>
          <el-table-column align="center" label="到期时间" min-width="120">
            <template slot-scope="scope">
              <span>{{ scope.row.endtime ?scope.row.endtime:''| scope.row.endtime }}</span>
            </template>
          </el-table-column>
        </el-table>
      </el-collapse-item>
      ...
      <el-collapse-item id="orderInfo" title="套餐订购记录" name="3">
        <el-table
          ...
          <el-table-column align="center" label="操作时间" min-width="125">
            <template slot-scope="scope">
              <span>{{ scope.row.addtime?scope.row.addtime:''| scope.row.endtime }}</span>
            </template>
          </el-table-column>

          <el-table-column align="center" label="开始时间" min-width="125">
            <template slot-scope="scope">
              <span>{{ scope.row.starttime?scope.row.starttime:''| scope.row.endtime }}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column align="center" label="退订时间" min-width="125">
            <template slot-scope="scope">
              <span>{{ scope.row.refund_time?scope.row.refund_time:''| scope.row.endtime }}</span>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
              ...
              res.data.rows.map((value, index) => {
                if (+value.status === 2 || +value.status === 3) {
                  ...
                  updteOrder({ id: v.id }).then(data => {
                    if (i === this.orderManagementInfo.changeArr.length - 1) {
                      loading.close()
                    }
    ...
    getCard() {
      ...
      const loading = this.$loading({ target: '#cardInfo' })
      getBindCard({
        card: this.card, uid: sessionStorage.getItem('uid')
      }).then(res => {
        this.$message({
    ...
    getStatus() {
      const loading = this.$loading({ target: '#statusInfo' })
      if (!this.card) {
    ...
    getChangePackage() {
      ...
      const loading = this.$loading({ target: '#changePackageInfo' })
      getchangeList({
        old_card: this.card
      }).then(res => {
    ...
    getOrderManagement() {
      ...
      const loading = this.$loading({ target: '#orderManagementInfo' })
      getOrder({
    ...
    getOrder() {
      ...
      const loading = this.$loading({ target: '#orderInfo' })
      getAddPackage({
        card: this.card
      }).then(res => {
        loading.close()
    ...
    getChange() {
      ...
      const loading = this.$loading({ target: '#changeInfo' })
      changeCard({
        oldCard: this.iccid
      }).then(res => {
        if (+res.data.total) {
    ...
    getPackage() {
      ...
      const loading = this.$loading({ target: '#packageInfo' })
      getInfo({
        card: this.card
      }).then(res => {
        loading.close()
