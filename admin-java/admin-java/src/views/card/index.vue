<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      ...
      <el-input v-show="secMore" v-model.trim="listQuery.remarks" class="filter-item" style="width:130px" clearable placeholder="请输入备注" @keyup.enter.native="handleFilter"/>
      ...
      <el-button v-waves v-permission="['kuyuplat:export:cardInfo']" class="filter-item" size="small" type="warning" @click="handleDownload">Java导出</el-button>
      <el-button v-waves v-permission="['kuyuplat:excel:batchCardAdd']" class="filter-item" size="small" type="warning " @click="cardJump('/card/add')">导卡</el-button>
      <el-button v-waves v-permission="['kuyuplat:excel:batchCardDivide']" class="filter-item" size="small" type="warning " @click="cardJump('/card/divide')">分卡</el-button>
      <el-button v-waves v-permission="['kuyuplat:excel:batchUpdateCardMoney']" class="filter-item" size="small" type="info" @click="cardJump('/card/recharge')">充值</el-button>
      <el-button v-waves v-permission="['kuyuplat:excel:cardBatchUpdate']" class="filter-item" size="small" type="info" @click="cardJump('/card/edit')">修改卡片</el-button>
      ...
      <el-button v-waves v-permission="['kuyuplat:work:search']" class="filter-item" size="small" type="success " @click="operateList">操作记录</el-button>
    </div>
    ...
    <el-table
      ...
      <el-table-column align="center" label="卡状态" show-overflow-tooltip min-width="100">
        <template slot-scope="scope">
          <el-tag
            ...
            @click.native="cardJump(`/business/stopReset?iccid=${scope.row.iccid}&cardId=${scope.row.id}&tel=${scope.row.tel}`)">
            {{ cardStatusMap[scope.row.cardStatus]||'未知状态' }}</el-tag>
        </template>
      </el-table-column>

      <el-table-column align="center" label="生命周期" show-overflow-tooltip min-width="100">
        <template slot-scope="scope">
          <el-tag :type="scope.row.lifeCycle==1||scope.row.lifeCycle==2?'':scope.row.lifeCycle==3?'success':'warning'">{{ lifeCycleMap[scope.row.lifeCycle]||'未知' }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column align="center" label="实名状态" show-overflow-tooltip>
        <template slot-scope="scope">
          <el-tag
            :type="scope.row.realStatus==1?'danger':'success'"
            @click.native="cardJump(`/card/AutonymDetails?iccid=${scope.row.iccid}&tel=${scope.row.tel}`)">{{ scope.row.realStatus==1?'未认证':'认证' }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('table.actions')" align="left" show-overflow-tooltip min-width="500px">
        <template slot-scope="scope">
