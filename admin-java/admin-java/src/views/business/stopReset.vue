<template>
  ...
  >
    <div class="filter-container">
      <el-input v-if="source!='mobile'" v-model.trim="listQuery.iccid" class="filter-item" style="width:220px" clearable placeholder="请输入ICCID" @change="handleChange" @keyup.enter.native="handleFilter"/>
      <el-input v-if="source!='mobile'" v-model.trim="listQuery.tel" class="filter-item" style="width:220px" clearable placeholder="请输入接入号" @change="handleChange" @keyup.enter.native="handleFilter"/>
    ...
    <el-table
      ...
      <el-table-column show-overflow-tooltip align="center" label="卡号" min-width="150">
        <template slot-scope="scope">
          <span>{{ scope.row.card }}</span>
        </template>
      </el-table-column>

      <el-table-column show-overflow-tooltip align="center" label="操作者" min-width="125">
        <template slot-scope="scope">
          <span>{{ scope.row.dname }}</span>
        </template>
      </el-table-column>

      <el-table-column show-overflow-tooltip align="center" label="操作类型">
        <template slot-scope="scope">
          <el-tag :type="scope.row.type==1?'danger':'success'">{{ scope.row.type==1?'关机':'开机' }}</el-tag>
        </template>
      </el-table-column>

      <el-table-column show-overflow-tooltip align="center" label="操作时间" min-width="125">
        <template slot-scope="scope">
