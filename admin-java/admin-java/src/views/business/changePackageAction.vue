    ...
    order() {
      ...
      for (const [index, elem] of oldArr.entries()) {
        ...
        setTimeout(() => {
          cardChange({
            cardnew: elem.trim(),
            cardold: newArr[index].trim(),
            copyagent: this.copyagent,
            difagent: this.difagent
          }).then(res => {
            for (const v of this.list) {
              if (v.cardold === cardold) {
                if (!v.id && v.id !== 0) {
                  const index = this.list.indexOf(v)
                  this.list.splice(index, 1, {
                    cardold, cardnew, status: res.status, message: res.message, difagent: this.difagent, id: index
                  })
                  break
                }
              }
            ...
            for (const v of this.list) {
              if (v.cardold === cardold) {
                if (!v.id && v.id !== 0) {
                  const index = this.list.indexOf(v)
                  this.list.splice(index, 1, {
                    cardold, cardnew, status: 2, message: '请求失败', difagent: this.difagent, id: index
                  })
                  break
                }
              }
