     ...
     row-class-name='row-class-name' cell-class-name='cell-class-name' -->
      ...
      <el-table-column :label="$t('table.actions')" show-overflow-tooltip align="center" min-width="280">
        <template slot-scope="scope">
          ...
          <el-button v-permission="['kuyuplat:packageStatus:update']" v-if="+scope.row.status=== 2" type="success" size="mini" @click="handleStatus(scope.row)">上架</el-button>
          <el-button v-permission="['kuyuplat:packageStatus:update']" v-if="+scope.row.status=== 1" type="warning" size="mini" @click="handleStatus(scope.row)">下架</el-button>
          <el-button v-permission="['kuyuplat:packageHide:update']" v-if="+scope.row.hide=== 2" type="success" size="mini" @click="handleHide(scope.row)">平台上架</el-button>
          <el-button v-permission="['kuyuplat:packageHide:update']" v-if="+scope.row.hide=== 1" type="warning" size="mini" @click="handleHide(scope.row)">平台下架</el-button>
