<template>
  <div class="app-container calendar-list-container">
    ...
    <tree-table :data="list" :expand-all="expandAll" :icon="true" :icon-sec="iconSec" :first-label="firstLabel" :toggle-expanded="toggleExpanded" :permission= "checkPermission(['kuyuplat:agent:launch'])" class="table" border>
      ...
      <el-table-column :label="$t('table.actions')" show-overflow-tooltip align="center" min-width="620">
        <template v-if="scope.row.type!=='agent'&&scope.row.type!=='kefu'" slot-scope="scope">
          ...
          <el-button v-permission="['kuyuplat:packagePrice:show']" v-if="scope.row.isper == 2" type="success" size="mini" @click="jump(`/agent/package?name=${scope.row.name}&aid=${scope.row.id}`)">套餐</el-button>
          ...
          <el-button v-permission="['kuyuplat:userMenu:distribute']" type="primary" size="mini" @click="handleUserRole(scope.row)">权限</el-button>
