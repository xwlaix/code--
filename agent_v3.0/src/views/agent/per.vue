...
<script>
import { getPer, updateAgent } from '@/api/agent'
...
export default {
  ...
  methods: {
    changeAid(value, operatorid) {
      this.items = []
      this.subordinate = this.per[+value] ? JSON.parse(this.per[value]) : ''
    ...
    refresh() {
      agentArr((res, bankCard, alipay, aname, per) => {
        this.agent = res
        this.per = Object.assign(this.superiorPer, per)
      })
    },
    getPerList() {
      this.refresh()
      ...
      this.$prompt('', `${this.project}--${row.key}`, {
        ...
        .then(({ value }) => {
          if (+value === +row.sub) {
            ...
            updateAgent({ id: this.listQuery.aid, isper: 2, per: updataePer }).then(
              res => {
                if (+res.code === 1) {
                  this.refresh()
                  row.sub = value
    ...
    handleDelete(row) {
      this.$confirm('此操作将重置该批次比例, 是否继续?', '提示', {
        ...
        .then(() => {
          ...
          updateAgent({ id: this.listQuery.aid, per: updataePer }).then(res => {
            if (+res.code === 1) {
              this.refresh()
              row.sub = 0
            }
